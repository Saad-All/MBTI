...
import { MBTIContentService, MBTIPersonalityContent } from "@/lib/services/MBTIContentService";
...
export const EnhancedSAISResultsDisplay: React.FC<EnhancedSAISResultsDisplayProps> = ({
  results,
  language,
  className = "",
}) => {
  const { t } = useTranslation();
  const [mbtiContent, setMbtiContent] = useState<MBTIPersonalityContent | null>(null);
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const content = contentService.getPersonalityContent(results.mbtiType, language);
  ...
  const calculateSAISPercentages = (score: DimensionScore): {
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-900 via-purple-800 to-blue-900 p-8 md:p-12 text-white">
        ...
        <div className="relative z-10">
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold mb-3">{mbtiContent?.arabicTitle || insights.arabicTitle}</h1>
            <p className="text-xl opacity-90 mb-2">{mbtiContent?.arabicSubtitle || insights.arabicSubtitle}</p>
          ...
          <div className="mt-8 p-6 bg-white/10 rounded-xl backdrop-blur-sm">
            ...
            <p className="text-sm opacity-90 mb-4">
              تم تقييم شخصيتك باستخدام منهجية التوزيع الخماسي المتقدمة (SAIS) التي تقيس عمق تفضيلاتك النفسية 
              عبر توزيع 5 نقاط لكل سؤال، مما يوفر دقة أكبر في فهم ميولك الحقيقية.
            </p>
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="bg-white/10 rounded-lg p-3">
                <strong>نظام التقييم:</strong> 15 نقطة لكل بُعد (3 أسئلة × 5 نقاط)
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 shadow-lg">
        ...
        <div className="space-y-6">
          {results.dimensionScores.map((score) => {
            ...
            return (
              <div key={score.dimension} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">
                  {dimensionNames[score.dimension as keyof typeof dimensionNames]}
                </h3>
                
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div className={`p-4 rounded-lg ${score.preference === percentages.optionA.letter ? 'bg-indigo-50 border-2 border-indigo-300' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-lg">{percentages.optionA.letter}</span>
                  ...
                  <div className={`p-4 rounded-lg ${score.preference === percentages.optionB.letter ? 'bg-indigo-50 border-2 border-indigo-300' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-lg">{percentages.optionB.letter}</span>
                      <span className="text-2xl font-bold text-indigo-600">
                ...
                <div className="relative h-8 bg-gray-200 rounded-full overflow-hidden">
                  ...
                  >
                    <span className="absolute left-2 top-1/2 transform -translate-y-1/2 text-white text-sm font-medium">
                      {percentages.optionA.letter}: {percentages.optionA.percentage}%
                    </span>
                  </div>
                  <div 
                  ...
                  >
                    <span className="absolute right-2 top-1/2 transform -translate-y-1/2 text-white text-sm font-medium">
                      {percentages.optionB.letter}: {percentages.optionB.percentage}%
                    </span>
                  </div>
                </div>
                
                <p className="text-sm text-gray-600 mt-3 text-center">
                  التفضيل الغالب: <strong className="text-indigo-600">{score.preference}</strong> بنسبة ثقة {score.confidence}%
                </p>
              </div>
            );
          })}
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          <h4 className="font-semibold text-blue-800 mb-2">كيف تم حساب نتائجك؟</h4>
          <p className="text-sm text-blue-700">
            في منهجية SAIS، قمت بتوزيع 5 نقاط لكل سؤال من أسئلة الوعي الـ 12. 
            كل بُعد من أبعاد شخصيتك يحتوي على 3 أسئلة، مما يعطي مجموع 15 نقطة لكل بُعد. 
            النسب المئوية أعلاه تعكس توزيعك الفعلي للنقاط وتُظهر قوة تفضيلاتك النفسية بدقة.
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
          ...
          <div className="space-y-3">
            {insights.innerGrowthRecommendations.map((recommendation, index) => (
...
const ConsciousnessDomainCard: React.FC<ConsciousnessDomainCardProps> = ({
  ...
  return (
    <div className={`bg-gradient-to-br ${colorClasses[color]} rounded-xl border p-6 hover:shadow-lg transition-all duration-300`}>
      <div className="flex items-center mb-4">
        ...
        <div className="w-full bg-gray-200 rounded-full h-4 mb-3 overflow-hidden">
          <div
            className={`h-full bg-gradient-to-r ${
              ...
              color === 'purple' ? 'from-purple-400 to-violet-500' :
              color === 'pink' ? 'from-pink-400 to-red-500' :
